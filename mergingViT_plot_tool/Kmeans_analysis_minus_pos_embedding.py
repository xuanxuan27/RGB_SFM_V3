...
def get_all_features_at_checkpoints(model, x, subtract_pos_embed=True):
    """
    一次 forward，收集每個 (stage, block) 後的特徵。
    避免對每個 checkpoint 重複跑整個模型。
    
    Args:
        subtract_pos_embed: 若 True，回傳前扣掉該 stage 的 pos_embeds，
                           讓 K-means 不要學到位置資訊。
    ...
    for i in range(len(model.stages)):
        ...
        for b, blk in enumerate(model.stages[i]):
            x = blk(x)
            feat = x.clone()
            if subtract_pos_embed:
                feat = feat - model.pos_embeds[i]  # [B,N,C] - [1,N,C]
            results[(i, b)] = (feat, H, W)
        if not isinstance(model.merges[i], nn.Identity):
            x, H, W = model.merges[i](x, H, W)
    return results
...
class ViTAnalyzer:
    def __init__(self, model, dataloader, img_size=28, device='cuda', subtract_pos_embed=True):
        self.model = model.to(device).eval()
        self.dataloader = dataloader
        self.device = device
        self.img_size = img_size
        self.subtract_pos_embed = subtract_pos_embed
    ...
    def extract_all_features_single_pass(self, max_samples=None):
        ...
        with torch.no_grad():
            for imgs, _ in self.dataloader:
                imgs = imgs.to(self.device)
                checkpoints = get_all_features_at_checkpoints(
                    self.model, imgs, subtract_pos_embed=self.subtract_pos_embed
                )
    ...
    def visualize_position_clusters(
        ...
        for pos in positions_to_show:
            fig, axes = plt.subplots(n_clusters, k_nearest + 1, figsize=(12, n_clusters * 1.5))
            if n_clusters == 1:
                axes = axes.reshape(1, -1)
            
            for c in range(n_clusters):
                axes[c, 0].text(0.5, 0.5, f'Pos {pos}\nCluster {c}', ha='center', va='center')
                axes[c, 0].axis('off')
                for i in range(k_nearest):
                    if (pos, c) in representatives and i < len(representatives[(pos, c)]):
                        axes[c, i + 1].imshow(representatives[(pos, c)][i])
                    axes[c, i + 1].axis('off')
            
            plt.suptitle(f'Position {pos} (row={pos//W}, col={pos%W})')
            plt.tight_layout()
            if save_path:
    ...
    def full_analysis_all_stages_blocks(
        ...
        """
        if save_dir is None:
            subdir = 'kmeans_all_stages_blocks_substract' if self.subtract_pos_embed else 'kmeans_all_stages_blocks'
            save_dir = Path(__file__).resolve().parent / 'plots' / subdir
        save_dir = Path(save_dir)
        save_dir.mkdir(parents=True, exist_ok=True)
    ...
    def _infer_and_save_m_images(
        ...
        with torch.no_grad():
            ...
            infer_checkpoints = get_all_features_at_checkpoints(
                self.model, imgs, subtract_pos_embed=self.subtract_pos_embed
            )
...
if __name__ == "__main__":
    ...
    run_colored_mnist_analysis_all(
        ...
        save_dir=None,   # 有扣 pos_embed 時預設 plots/kmeans_all_stages_blocks_substract/
